...
namespace CacheGenerator.Network;

public static class NetworkMessageGenerator
{
    private const string CacheNamespace = "Imlight.Common.Serializable.Caches";
    ...
    private static readonly Dictionary<string, (Type, DmlType)> InternalTypeTranslationDict = new()
    {
        { "BYT",    (typeof(sbyte),   DmlType.BYT)           },
        { "BOOL",   (typeof(bool),    DmlType.BOOL)          },
        ...
        { "STR",    (typeof(ByteString),  DmlType.STR)       },
        { "WSTR",   (typeof(WideByteString),  DmlType.WSTR)  },
    ...
    /// </summary>
    /// <param name="protocolClass"></param>
    /// <param name="xmlRecords"></param>
    /// <param name="serviceId"></param>
    /// <returns>A dictionary, where the key is the serviceId for a message, and the string is the name of
    /// the message.</returns>
    private static Dictionary<byte, string> AddSubRecordsToProtocol(
        ref CodeTypeDeclaration protocolClass, IEnumerable<XmlNode> xmlRecords, byte serviceId)
    {
    ...
    /// <summary>
    /// Creates a CodeTypeDeclaration from the provided ProtocolHeader.
    /// </summary>
    /// <param name="protocolHeader"></param>
    /// <returns></returns>
    ...
    /// <summary>
    /// Creates a CodeTypeDeclaration from the provided XML node.
    /// </summary>
    /// <param name="xmlNode"></param>
    /// <param name="serviceId"></param>
    /// <param name="messageOrderFallback"></param>
    /// <returns></returns>
    private static CodeTypeDeclaration CreateRecordTypeDeclaration(
        XmlNode xmlNode, byte serviceId, byte messageOrderFallback = 0)
    {
        ...
        for (var i = 0; i < xmlRecord!.ChildNodes.Count; i++)
        {
            ...
            var formattedType = type.Item1.ToString().Split('.').Last();
            var formattedAttributeName = nameof(DmlElementAttribute).Replace("Attribute", "");
            var propSnippetText =
    ...
    /// <summary>
    /// Implements the <see cref="INetworkProtocol"/> interface on the provided CodeTypeDeclaration.
    /// </summary>
    /// <param name="codeClass"></param>
    /// <param name="protocolHeader"></param>
    private static void ImplementNetworkProtocolInterfaceProperties(
        ref CodeTypeDeclaration codeClass, ProtocolHeader protocolHeader)
    {
    ...
    /// <summary>
    /// Implements the <see cref="INetworkMessage"/> interface on the provided CodeTypeDeclaration.
    /// </summary>
    /// <param name="codeClass"></param>
    /// <param name="xmlNode"></param>
    /// <param name="serviceId"></param>
    /// <param name="messageOrderFallback"></param>
    private static void ImplementNetworkMessageInterfaceProperties(
        ref CodeTypeDeclaration codeClass, XmlNode xmlNode, byte serviceId, byte messageOrderFallback)
    {
    ...
    private static void ImplementNetworkProtocolInterfaceDispatcher(
        ref CodeTypeDeclaration codeClass, Dictionary<byte, string> createdClasses)
    {
        /*
        ...
        */
        ...
        tw.WriteLine($"\n\n{ProtocolTabLength}public {nameof(INetworkMessage)} Dispatch(byte id)");
        tw.WriteLine("{");
        tw.Indent++;
        ...
        var dispatcherProp = CreateGenericPropertySnippet(sw.ToString());
        codeClass.Members.Add(dispatcherProp);
