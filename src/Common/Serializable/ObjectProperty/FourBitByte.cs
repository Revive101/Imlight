...
public struct FourBitByte : IConvertible
{
    private byte _value;

    public byte Value { get => _value; set => _value = value; }
    public FourBitByte(byte value) => _value = value;

    public static explicit operator FourBitByte(byte value) => new FourBitByte(value);
    public static implicit operator byte(FourBitByte bits) => bits._value;
