...
public struct SevenBitByte : IConvertible
{
    private byte _value;

    public byte Value { get => _value; set => _value = value; }
    public SevenBitByte(byte value) => _value = value;

    public static explicit operator SevenBitByte(byte value) => new SevenBitByte(value);
    public static implicit operator byte(SevenBitByte bits) => bits._value;
